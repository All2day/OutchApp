...
class Zend_OpenId_Consumer
{
                ...
                if (isset($params['openid_claimed_id'])) {
                    ...
                    if (!Zend_OpenId::normalize($id)) {
                        ...
                        return false;
                                ...
                                preg_replace("/#.*/","",$params["openid_identity"]) != preg_replace("/#.*/","",$id)) ||
    ...
    protected function _discovery(&$id, &$server, &$version)
    {
        ...
        if (preg_match(
            ...
            $version = 1.1;
            $server = $r[2];
        } else if (preg_match('/<URI>([^<]+)<\/URI>/i', $response, $r)) { //ADDED by kezzel to add support for google, see http://stackoverflow.com/questions/741345/how-do-i-implement-direct-identity-based-openid-authentication-with-zend-openid/844350#844350
  					$version = 2.0;
  					$server = $r[1];
  			} else {
            return false;
        ...
        $extensions=null, Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if ($version <= 2.0) {
            //ADDED by kezzel to add support for google, see http://stackoverflow.com/questions/741345/how-do-i-implement-direct-identity-based-openid-authentication-with-zend-openid/844350#844350
		        if ($server == 'https://www.google.com/accounts/o8/ud') {
						  $claimedId = $id = 'http://specs.openid.net/auth/2.0/identifier_select';
						}
            if ($this->_session !== null) {
